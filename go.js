//NOT MINE, IDK WHERE I FOUND IT, HOWEVER I DID EDIT IT A LOT
/** @param {NS} ns **/
...
export async function main(ns) {
  ns.tail();
    ...
    //}
    let targets = ["n00dles", "foodnstuff", "iron-gym", "sigma-cosmetics"]
		for (let server of servers) {
        ...
        //ns.tail("info.js", "home", target);
              ...
              ns.print(`Weakening ${target} on ${server} ${available_threads} times.`)
					...
					// grow the target while money < maxmoney
		  			if (available_threads >= 1) {
			  			ns.exec("gr.js", server, available_threads, target)
              ns.print(`Growing ${target} on ${server} ${available_threads} times.`)
				  	}
  				} else {
