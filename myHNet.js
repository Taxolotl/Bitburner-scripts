...
export async function main(ns) {
    // Maximum number of nodes to purchase
    const nodeNum = ns.args[0];
    /*
...
*/
    ...
    if (
        ...
        ns.tprint("Purchased a new hacknet node!");
        displayDashBoard(ns);
    }
        ...
        while (ns.getServerMoneyAvailable("home") >= reserveMoney) {
            for (var i = 0; i < ns.hacknet.numNodes(); i++) {
                while (
                    ...
                    //ns.tprint(`Upgraded level of node ${i} ${n} times.`)
                    displayDashBoard(ns);
                    await ns.sleep(100);
                }
                while (
                    ...
                    //ns.tprint(`Upgraded RAM of node ${i} ${n} times.`)
                    displayDashBoard(ns);
                    await ns.sleep(100);
                }
                while (
                    ...
                    //ns.tprint(`Upgraded core of node ${i} ${n} times.`)
                    displayDashBoard(ns);
                    await ns.sleep(100);
                }
            ...
            */
            if (
                ...
                if (ns.hacknet.numNodes() < nodeNum) {
                    ns.hacknet.purchaseNode();
                    //ns.tprint("Purchased a new node!");
                    displayDashBoard(ns);
                }
            } else if (
                /*
...
function displayDashBoard(ns) {
    ...
    let maxNodes = ns.hacknet.numNodes() < ns.args[0] ? ns.args[0] : Infinity
    ...
    //ns.print(`Total Spent: ${ns.hacknet.moneySpent()}`);
    ns.print(table(
